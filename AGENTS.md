...
- **Never break the persisted Y.Doc format silently.** Boards live in users' IndexedDB and in consumers' databases. The Y.Doc carries a `schemaVersion` (`DOC_SCHEMA_VERSION`): field changes stay **additive** (no bump); a **breaking layout change** bumps it and ships a migration, so a newer build still reads older documents ("N+1 reads N") while an older build **refuses** a newer document (`WhiteboardSchemaVersionError`) instead of mis-reading. See the package README § "Document format & compatibility".
