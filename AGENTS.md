...
Downstream consumers (including the private memorii product) install the npm package and gate version bumps with their own contract tests: **the exports map of `packages/processii/src/index.ts` is the public API**. Treat any change to it as a semver event.
...
3. Read `apps/standalone/README.md` before touching the app (sessions P2P, persistence, AI assistant).
4. `CONTRIBUTING.md` covers the PR process and DCO.

## Hard rules

- **Everything in English** — code, comments, tests labels, docs, commits, PR text. (The standalone app's user-facing UI copy is currently French: that is product copy, do not translate it as a side effect.)
...
- **The core stays DOM-free.** Only the React layer (`*.tsx` files) may touch React/DOM. `scene`, `board`, `engine`, `render`, `viewport`, `hit-test`, `handles`, `snap`, `history`, `connector`, `crdt`, `adapters` must run headless.
...
- `editor.tsx`, `board-canvas.tsx`, `toolbar.tsx`, `style-panel.tsx`, `side-panel.tsx`, `presence.ts` — the React editing surface (`WhiteboardEditor`).
