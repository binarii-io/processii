...
// jsdom does not implement these APIs used by some ui-kit Radix primitives / the canvas.
...
// `MouseEvent` (which does carry these fields) + `pointerId`, to test the canvas gestures.
...
// in jsdom); collab convergence and business logic are tested outside the rendering (board, engine, fake-transport).
