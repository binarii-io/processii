...
| `link_subprocess`   | `{ id, ref, kind?: 'sub' \| 'external' }`                                         | `{ id }` (links the host document `ref` to step `id`; `kind` is indicative — omitted, a re-link keeps the current label, a fresh link resets it) |
